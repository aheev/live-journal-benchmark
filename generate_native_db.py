...
EDGE_BATCH = 50_000      # edges per UNWIND MATCH CREATE batch
MAX_DB_SIZE = 32 * 1024**3  # 8 GB — default 1 GB is too small for this dataset
...
def load_edges(conn: lbug.Connection, edges_csv: str) -> int:
    """Insert edges via batched UNWIND+MATCH+CREATE (no COPY FROM, avoids hang with hash index disabled).
    ...
    def flush() -> None:
        nonlocal inserted
        pairs = "[" + ",".join(f"[{s},{d}]" for s, d in batch) + "]"
        conn.execute(
            f"UNWIND {pairs} AS p "
            f"MATCH (a:user {{id: p[0]}}), (b:user {{id: p[1]}}) "
        ...
        eta = (total - inserted) / rate if rate > 0 else 0
        print(
            f"\r  {inserted:,}/{total:,} edges  ({rate:,.0f}/s  ETA {eta:.0f}s)  ",
            end="",
            flush=True,
        )
    ...
    print(f"\r  {inserted:,} edges loaded in {time.perf_counter() - t0:.1f}s" + " " * 20)
    return inserted
...
def generate(db_path: str, nodes_csv: str, edges_csv: str) -> None:
    ...
    # ── Insert edges via UNWIND+MATCH+CREATE batches ──────────────────────────
    print(f"Loading edges from {edges_csv} ...")
    num_edges = load_edges(conn, edges_csv)
